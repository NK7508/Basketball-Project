...
function setup() {
  ...
  Hoop = new BasketballHoop(120, 330, 200, 200);
}
...
function draw() {
  ...
  for (var i = 0; i < playerShots.length; i++) {
    ...
    if (playerShots[i] !== undefined) {
      ...
      var posX = playerShots[i].body.position.x;
      var posY = playerShots[i].body.position.y;

      if (posX > width || posY > height) {
        if (!playerShots[i].isRemoved) {
          playerShots[i].remove(i);
        } else {
          playerShots[i].trajectory = [];
        }
      }
    }
  }
...
function keyPressed() {
  if (keyCode === 32) {
    if (numberOfShots > 0) {
      ...
      var ball = new Ball(posX, posY, 10, angle);
